    ...
    def run(self):
        try:
            ...
            if True:
                ...
                try:
                    ...
                    if video_embeddings_matrix is not None:
                        try:
                            self.log.emit("Đang dùng GPU nhúng toàn bộ phụ đề bằng AI (Batching)...")
                            ...
                            if len(text_embeddings) > 0:
                                ...
                                self.log.emit(f"Đã nhúng xong {len(all_texts)} câu phụ đề.")
                        except Exception as e:
                            self.log.emit(f"Cảnh báo: Lỗi khi nhúng phụ đề hàng loạt: {e}. Sẽ chạy ở chế độ nhúng từng câu.")
            ...
            for i, sub in enumerate(subs):
                ...
                if video_embeddings_matrix is not None:
                    try:
                        # Nhúng câu SRT (sử dụng ma trận tính sẵn nếu có)
